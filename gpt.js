document.addEventListener("DOMContentLoaded",()=>{ 
          ...
          <div class='card'>
              ...
              <div class="text">
                  <div class='te1'>
                      ...
                      <div class='border'>
                          <p class='bordercountry'>Border Countries:</p>
                      </div>
                  </div>
                  <div class='second'>
                      <p>Languages: ${Object.values(languages).join(', ')} </p>
                      <p>Currencies: ${Object.values(currencies)[0].name}</p>
                      <p>Top Level Domain: ${Object.values(tld).join(', ')}</p>
                  </div>
              </div> 
          </div>
      `;
  ...
  async function displayBorder(border){
      ...
      try {
          for (const code of border) {
              const data = await fetch(`https://restcountries.com/v3.1/alpha/${code}`);
              const result = await data.json();
              const {name:{common}} = result[0];
              displayBorderCountry.innerHTML += `
                  <a href="detpage.html?official=${common}"><button class='borderbutton'>${common}</button></a>
              `;
          }
      } catch (err){
